...
** Context-free grammar (ALGOL-derived syntax)
Most modern programming languages (C, Java, Ruby, Perl)  are designed with context-free grammars. They are often called C-like languages, or ALGOL-family languages. 
#+BEGIN_QUOTE
ALGOL 68 was defined using a two-level grammar formalism invented by Adriaan van Wijngaarden and which bears his name. Van Wijngaarden grammars use a context-free grammar to generate an infinite set of productions that will recognize a particular ALGOL 68 program. — Wikipedia
#+END_QUOTE
** Lisp takes off
